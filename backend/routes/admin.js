    ...
    try{
        ...
        if(rootUser){
            if(rootUser.otp != otp){
                ...
                await User.updateOne({email}, {$set: {password:hashed_password, confirm_password:hashed_confirm_password, otp:"0"}});
...
router.post("/api/forgot-password", async (req, res) =>{
    ...
    try{
        ...
        transporter.sendMail(options, function(err, info){
            if(err){
                res.status(401).json({error: "Could not send OTP to the given email"});
                console.log(err);
                return;
            }
...
router.post("/api/change-password-restaurant", async(req, res) =>{
    const {otp, password, confirm_password, email} = req.body;
    ...
    try{
        ...
        if(rootRestaurant){
            if(rootRestaurant.otp != otp){
                ...
                await Restaurant.updateOne({email}, {$set: {password:hashed_password, confirm_password:hashed_confirm_password, otp:"0"}});
        ...
        return res.status(401).json({error: "Email doesn't match with any restaurant"});
    }
})

router.post("/api/forgot-password-restaurant", async (req, res) =>{
    const {email} = req.body
